...
discard all NWB/AllenSDK access entirely.
...
**Why VISp only.** Both consumers (Parts 1.2 and 7.1) only ever operate on VISp in
the original analysis. Storing this table for all five areas would multiply that
cost fivefold for zero use.

## 4. Strategy: one global unit-metadata table instead of per-session metadata files
...
NWB file needs to be touched again, only the small derived table needs
recomputing.

## 5. Strategy: derived/expensive-to-recompute outputs are cached only where they're consumed, not centrally
...
## 6. Strategy: downsample images for the gallery, never use them for analysis
...
embeddings are computed once here and cached separately at full resolution
*before* downsampling the display copy). Downsampling only the copy intended for
visualization avoids inflating the archive for a use case that doesn't need the
extra resolution.

## 7. Strategy: per-area, per-stimulus grouping inside one HDF5 file rather than separate files

**Decision.** All binned tensors live in a single `responses.h5`, organized as
nested groups (`natural_scenes/{area}/{session_id}/tensor`,
`static_gratings/VISp/{session_id}/tensor`), rather than one file per
...
## 8. Strategy: a manifest.json as the single source of truth for reconstruction parameters

**Decision.** Bin edges, QC thresholds, session lists per area, and the window
reconstruction formula are all written to one `manifest.json`, rather than being
re-declared as constants inside each part-notebook.
...
## 9. Strategy: explicit del + gc.collect() after every session's extraction
...
tier. Without explicit cleanup, AllenSDK's `Session` objects and per-unit spike-time
dictionaries accumulate across 32+ iterations of the loop and can exceed available
RAM well before the loop finishes — this was a real failure mode in the original
single-notebook version when sections were combined. Discarding each session's raw
objects the moment its derived data is safely written keeps peak memory bounded by
one session's worth of raw data, not all 32+.

## 10. What is deliberately *not* cached, and why

- **Raw NWB files** — deleted at the end (Cell 15). Nothing downstream needs them
  once every raw quantity has been extracted into the archive; keeping them would
  multiply the deliverable size by roughly 10–20x for no benefit.
- **Full-resolution natural scene images for analysis** — see §6; only a display
  copy is kept.
...
- **RSU/FSU consistency curves, E/I model fits, any other Part 5+ analysis output**
  — see §5; these are analysis results, not raw extractions, and belong in the
...
## 11. Summary: the dividing line this notebook enforces

Everything above follows from one rule, applied consistently: **this notebook's
output should make every part-notebook's only data dependency be array slicing,
dataframe filtering, or reading a small derived artifact from a prior part — never
AllenSDK, never h5py spike-time access, never NWB files.** Any time a part-notebook
needs to "go back to the raw data" for something not already in the archive, that's
a signal this preprocessing notebook is missing an extraction, not that the
part-notebook should make its own NWB calls.
